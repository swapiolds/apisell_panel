...
if($key === ''){
    echo json_encode([
        ...
        'usage' => '?key=YOUR_KEY&dl_no=UP2520210027727&dob=01-01-2001',
        'BUY_API' => '@swapibhai',
        'SUPPORT' => '@swapibhai'
...
if(!in_array($type, $allowed_services)){
    echo json_encode([
        'error' => 'Key is not authorized for DL Info API',
        'BUY_API' => '@swapibhai',
        'SUPPORT' => '@swapibhai'
    ], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    exit;
...
// Validate input
$dl_no = isset($_GET['dl_no']) ? trim(strtoupper($_GET['dl_no'])) : '';
$dob = isset($_GET['dob']) ? trim($_GET['dob']) : '';
if($dl_no === '' || $dob === ''){
    echo json_encode(['error' => 'Missing dl_no or dob parameter', 'usage' => '?key=YOUR_KEY&dl_no=UP2520210027727&dob=01-01-2001'], JSON_PRETTY_PRINT);
    exit;
}
$query = $dl_no;
...
$url = 'https://bossapi.vishalhub.store/dlinfo.php?dl_no=' . urlencode($dl_no) . '&dob=' . urlencode($dob);
...
$failed = false;
if(isset($data['error']) || empty($data) || (isset($data['status']) && $data['status'] !== true && strtolower((string)$data['status']) !== 'true' && strtolower((string)$data['status']) !== 'success')) {
    $failed = true;
}

if($failed){
    echo json_encode(['error' => $data['message'] ?? $data['error'] ?? 'DL data not found', 'status' => 'failed', 'BUY_API' => '@swapibhai', 'SUPPORT' => '@swapibhai'], JSON_PRETTY_PRINT);
    exit;
}
...
if($data && isset($data['data'])) {
    ...
    if(isset($data['data']['photo_url']) && strpos($data['data']['photo_url'], 'https://bossapi.vishalhub.store') !== false){
        $img_name = basename($data['data']['photo_url']);
        $data['data']['photo_url'] = $base_url . '/photo.php?img=' . urlencode($img_name);
    }
    if(isset($data['data']['signature_url']) && strpos($data['data']['signature_url'], 'https://bossapi.vishalhub.store') !== false){
        $img_name = basename($data['data']['signature_url']);
        $data['data']['signature_url'] = $base_url . '/photo.php?img=' . urlencode($img_name);
    }
}
