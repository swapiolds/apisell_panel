...
if($key === ''){
    echo json_encode([
        ...
        'usage' => '?key=YOUR_KEY&num=DL10CE0409',
        'BUY_API' => '@swapibhai',
        'SUPPORT' => '@swapibhai'
...
if(!in_array($type, $allowed_services)){
    echo json_encode([
        'error' => 'Key is not authorized for Vehicle Info API',
        'BUY_API' => '@swapibhai',
        'SUPPORT' => '@swapibhai'
    ], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    exit;
...
// Validate input
$num = isset($_GET['num']) ? trim(strtoupper($_GET['num'])) : '';
if($num === ''){
    echo json_encode(['error' => 'Missing num parameter', 'usage' => '?key=YOUR_KEY&num=DL10CE0409'], JSON_PRETTY_PRINT);
    exit;
}
$query = $num;
...
$url = 'https://srbhai.shubhamv189.workers.dev/?number=' . urlencode($num);
...
if($failed){
    echo json_encode(['error' => $data['error'] ?? 'Vehicle data not found', 'status' => 'failed', 'BUY_API' => '@swapibhai', 'SUPPORT' => '@swapibhai'], JSON_PRETTY_PRINT);
    exit;
}
