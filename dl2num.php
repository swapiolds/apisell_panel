...
if(!in_array($type, $allowed_services)){
    echo json_encode([
        'error' => 'Key is not authorized for DL To Number API',
        'BUY_API' => '@swapibhai',
        'SUPPORT' => '@swapibhai'
    ], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    exit;
...
$url = 'https://bossapi.vishalhub.store/dl2num.php?dl_no=' . urlencode($dl_no) . '&dob=' . urlencode($dob);
...
if($failed){
    echo json_encode(['error' => $data['message'] ?? $data['error'] ?? 'DL to number data not found', 'status' => 'failed', 'BUY_API' => '@swapibhai', 'SUPPORT' => '@swapibhai'], JSON_PRETTY_PRINT);
    exit;
}
