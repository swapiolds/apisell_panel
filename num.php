...
if(!in_array($type, $allowed_services)){
    echo json_encode([
        'error' => 'Key is not authorized for Number Info API',
        'BUY_API' => '@swapibhai',
        'SUPPORT' => '@swapibhai'
    ], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    exit;
...
if($num === ''){
    echo json_encode(['error' => 'Missing num parameter', 'usage' => '?key=YOUR_KEY&num=9876543210'], JSON_PRETTY_PRINT);
    exit;
}
...
$failed = false;
if(isset($data['error']) || empty($data) || (isset($data['status']) && $data['status'] === 'failed')) {
    $failed = true;
}

if($failed){
    // Refund/Fail Logic - No limits deducted on failure
    echo json_encode(['error' => $data['error'] ?? 'Data not found', 'status' => 'failed', 'BUY_API' => '@swapibhai', 'SUPPORT' => '@swapibhai'], JSON_PRETTY_PRINT);
    exit;
}
